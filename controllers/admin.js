...
exports.addRule = asyncHandler(async (req, res, next) => {
    ...
    if (type === "uses" || type==="privacy"||type==="wallet") {
        ...
        const banner_imgs = await Promise.all(req.files.map(file => cloudinary.uploader.upload(file.path)))
        ...
        await Rules.findOne({ type }).then(async rule => {
          if (rule) {
            await Rules.findOneAndUpdate({ type }, { banner_img: banner_imgs }, { new: true })
              .then(banner => res.json({ banner }));
          } else {
            await Rules.create({ banner_img: banner_imgs, type })
              .then(banner => res.json({ banner }));
          }
        });
