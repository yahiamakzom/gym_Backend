...
async function refreshSubscriptions() {
  ...
  const subscriptions = await Subscription.find({ type: { $in: subscriptionTypes } });

  const clubs = await Club.find({});

  for (const club of clubs) {
    const clubSubscriptions = subscriptions.filter(subscription => subscription.club.toString() === club._id.toString());

    if (club.to && club.from) {
      const toTime = moment(club.to, "HH:mm");
      const taskTime = moment().set({ hour: toTime.hours(), minute: toTime.minutes(), second: 0, millisecond: 0 });

      scheduleTask(taskTime, clubSubscriptions);
    } else {
      const endOfDay = moment().endOf('day').set({ hour: 23, minute: 59, second: 0, millisecond: 0 });
...
function scheduleTask(time, subscriptions) {
  const job = new cron.CronJob(time.format('ss mm HH DD MM *'), async function () {
    console.log(`Task scheduled at: ${time.format('HH:mm')}`);
